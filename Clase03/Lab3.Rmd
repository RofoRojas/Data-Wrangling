...
nuevo_df %>% 
  ...
  mutate(Por_Falt=Falt/Tot) %>% arrange(desc(Por_Falt)) %>% 
  mutate(mayor= ifelse(Por_Falt>=mean(Por_Falt), 1, 0)) %>% 
  ungroup() %>% group_by(PILOTO) %>% 
  summarise(mes_Raro= sum(mayor)) %>% arrange(desc(mes_Raro)) %>% View()
```
```{r}
nuevo_df %>% 
...
nuevo_df %>% 
  group_by(PILOTO) %>% 
  summarise(Tot=n(), Falt=sum(Faltante), Dev=sum(Devolucion)) %>% 
  mutate(Por_Falt=Falt/Tot*100) %>% arrange(desc(Por_Falt)) %>% 
  mutate(mayor= ifelse(Por_Falt>=mean(Por_Falt), 1, 0)) %>% 
  View()
```
